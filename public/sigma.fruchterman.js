...
sigma.fruchtermanReingold.FruchtermanReingold = function (inst, fixedNodes) {
  sigma.classes.Cascade.call(this);
  var self = this;
  this.inst = inst;
  this.graph = inst._core.graph;
  this.stop = false;
  ...
  this.atomicGo = function () {
    ...
    if (sumDist < (Math.sqrt(nodes.length) / 100) && diffDist < (Math.sqrt(nodes.length) / 100)) {
    	self.p.speed *= 0.995;
    	if (self.p.speed < 0.06) {
    		self.stop = true;
        inst.removeGenerator('FruchtermanReingold');
        inst.activateFishEye().draw();
    	}
    }
    oldSumDist = sumDist;
...
sigma.publicPrototype.startFruchtermanReingold = function () {
  this.FruchtermanReingold = new sigma.fruchtermanReingold.FruchtermanReingold(this);
